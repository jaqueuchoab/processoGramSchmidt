		...
		if (validarBaseOrtogonal(base) == true) {
			for (let i = 0; i < base.length; i++) {
				for (let j = 0; j < base.length; j++) {
					if (i == j) {
						if (
							math.round(calcularNormaVetor(base[i], base[j])) !=
							1
						) {
...
function calcularRazaoGramSchmidt(baseV, baseW) {
	wAtual =
...
function calcularBaseOrtogonal(base) {
	if (validarBaseOrtogonal(base) == false) {
		...
		while (i < base.length) {
			if (i == 1) {
				wDefinitivo = calcularRazaoGramSchmidt(base[i], novaBase[i - 1]);
				wDefinitivo = calcularSubtracaoVetores(base[i], wDefinitivo);
				novaBase.push(wDefinitivo);
			} else {
				let k = 0;
				while (k < i) {
					wAtual = calcularRazaoGramSchmidt(base[i], novaBase[k]);
					
					if (k == 0) {
						wDefinitivo = wAtual;
						wDefinitivo = calcularSubtracaoVetores(base[i], wDefinitivo);
					} else
						wDefinitivo = calcularSubtracaoVetores(wDefinitivo, wAtual);
...
if (validarBaseOrtogonal(base) == false) {
	base = calcularBaseOrtogonal(base);
	console.log("Base ortogonalizada: ", base);
}

console.log('É ortonormal?', validarBaseOrtonormal(base));
if (validarBaseOrtonormal(base) == false) {
	base = calcularBaseOrtonormal(base);
	console.log("Base ortonormalizada: ", base);
}
